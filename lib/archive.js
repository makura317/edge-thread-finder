...
export function parseArchiveJsonl(text) {
  return text.trim().split(/\r?\n/).filter(Boolean).map(line => JSON.parse(line)).map(thread => ({
    ...
    opHeader: firstResponseHeader(thread.body),
...
function firstResponse(body) {
  ...
  const header = /^1(?:\s+\(\d+\))?\s*:\s+[\s\S]*?\s+\d{4}\/\d{2}\/\d{2}\([^)]*\)\s+\d{2}:\d{2}:\d{2}\.\d+\s+\S+(?:\s+\(\d+\/\d+\))?\s+/;
  const opening = header.test(body) ? body.replace(header, '') : body.replace(/^1(?:\s+\(\d+\))?\s*:\s*/, '');
  const nextResponse = opening.search(/\s+2(?:\s+\(\d+\))?\s*:\s+[\s\S]*?\s+\d{4}\/\d{2}\/\d{2}\(/);
  return (nextResponse === -1 ? opening : opening.slice(0, nextResponse)).trim();
}

function firstResponseHeader(body) {
  const legacy = body.match(/^1(?:\s+\(\d+\))?\s*:\s+([\s\S]*?)\s+(\d{4}\/\d{2}\/\d{2}\([^)]*\)\s+\d{2}:\d{2}:\d{2}\.\d+)\s+(\S+)/);
  if (!legacy) return '';
  const [, name, timestamp, id] = legacy;
  return `1 ${name.trim()} ${timestamp.replace(/^\d{4}\//, '')} ID:${id}`;
}
